...
class tics:
    def tic1(y):
        ...
        if ty>thresh and qq>5 or qq>25:
            ...
            colm = np.linspace(0,config.N_PIXELS // 2-1, np.floor(mu).astype(int)).astype(int)
            hg =0 
            qq = 0
    ...
    def tic2(y, lpad):
        ...
        if coms[273] == 1: #Arrow Up
            coms[273] = 0
            thresh*=1.1
            print('Threshold Up, tic2: %5.3f.' % thresh)
        if coms[274] == 1: #Arrow Down
            coms[274] = 0
            thresh*=.9
            print('Threshold Down, tic2: %5.3f.' % thresh)
        
        if ty>thresh and qq2>40:
            color = pallette.pal(0)
            qq2 = 0
           
        if ty>thresh or qq>40 or coms[275] == 1 or lpad[1,56] == 1:
            if coms[275] == 1 or lpad[1,56] == 1:
                tic_yn = 1
                coms[275] = 0
                lpad[1,56] = 0
        ...
        if tic_yn == 1:
            ...
            for x in np.linspace(0,len(color[0,:])-1,len(color[0,:])).astype(int):
                colm = np.linspace(0,config.N_PIXELS // 2-1, nu+x).astype(int)
                p[0,colm] = color[0,x]
                p[1,colm] = color[1,x]
                p[2,colm] = color[2,x]
    ...
    def ticpal(y):
        ...
        if ty>thresh or qq>30  or coms[275] == 1: #Right arrow
            if coms[275] == 1:
                lpad[1,61] = 0
                p[:,:] = 0
