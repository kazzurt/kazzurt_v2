...
# Raspberry Pi controls the LED strip directly
    ...
    #strip2 = Adafruit_NeoPixel(config.N_PIXELS, config.LED_PIN2,
                                      #config.LED_FREQ_HZ, config.LED_DMA2,
                                      #config.LED_INVERT, config.BRIGHTNESS)
    strip.begin()
    #strip2.begin()
...
def _update_pi():
    ...
    for i in range(len(p[0,:])):  #config.N_PIXELS
        if len(p[0,:]) == len(_prev_pixels[0,:]):
            # Ignore pixels if they haven't changed (saves bandwidth)
            if np.array_equal(p[:, i], _prev_pixels[:, i]):
                continue
            
        strip._led_data[i] = int(rgb[i])
        #strip2._led_data[i] = int(rgb[i])
    _prev_pixels = np.copy(p)
    strip.show()
    #strip2.show()
