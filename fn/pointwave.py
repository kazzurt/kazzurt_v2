...
print(arx)
print(arx2)
ary         = np.linspace(0,11,12).astype(int)
...
class pointwave:
    def pointwave(y):  
        ...
        xdi = 5*np.sin(np.pi*rtim/25)+6
        ydi = 10*np.sin(np.pi*rtim/25)+15
        ...
        for i in arx2:    
            gre_ar2[i,ary] =  (.5*np.sin(-rtim/4 - (ary+1)/ydi + i/xdi            )+.4)
            blu_ar2[i,ary] =  (.5*np.sin(-rtim/4 - (ary+1)/ydi + i/xdi - 2*np.pi/3+phas)+.4)
            red_ar2[i,ary] =  (.5*np.sin(-rtim/4 - (ary+1)/ydi + i/xdi - 4*np.pi/3+2*phas)+.4)  
#         for i in arx:
...
#             blu_ar2[i,ary] =  (.5*np.sin(rtim/sec + ary/ydi + i/xdi )+.5)
        ...
        p[0,:] = viz_mf.flatMatHardMode(red_ar)+viz_mf.flatMatHardMode(np.fliplr(red_ar2)) #np.fliplr(r 
        p[1,:] = viz_mf.flatMatHardMode(gre_ar)+viz_mf.flatMatHardMode(np.fliplr(gre_ar2))
        p[2,:] = viz_mf.flatMatHardMode(blu_ar)+viz_mf.flatMatHardMode(np.fliplr(blu_ar2))
        
        #p     *=  viz_mf.flatMatHardMode(inten1)# + viz_mf.flatMatHardMode(inten2)/2
        p = p/np.max(p)*255
        return p
