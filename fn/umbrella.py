...
class umbrella1:
    def umbrella1(y):
            ...
            cnt5+=1
            p = .2*np.fliplr(colorwave1.colorwave1(y))
            for i in range(0,len(umbrella)):
                if i%2 == 1:
                    
                    p[0,umbrella[i]] = int(.5*(np.sin(cnt5/25)+1)*255)
                    p[0,umbrella[i]] = int(.5*(np.sin(cnt5/25+np.pi/2)+1)*255)
                    p[1,umbrella[i]] = 0#int(.5*(np.sin(cnt5/25+phum[1])+1)*255)
                else:
                    p[0,umbrella[i]] = int(.5*(np.sin(cnt5/25)+1)*255)
                    p[0,umbrella[i]] = int(.5*(np.sin(cnt5/25+np.pi/2)+1)*255)
                    p[2,umbrella[i]] = int(.5*(np.sin(cnt5/25)+1)*255)
