...
class colorwave7:
    ...
    def colorwave7(y):
            ...
            p[0,:] = (.5*np.sin(rtim11/40/4)+.5)*coo11[0]*viz_mf.flatMatHardMode(red_ar)
            p[1,:] = (.5*np.sin(rtim11/40/4+2*np.pi/3)+.5)*coo11[1]*viz_mf.flatMatHardMode(gre_ar)
            p[2,:] = (.5*np.sin(rtim11/40/4+4*np.pi/3)+.5)*coo11[2]*viz_mf.flatMatHardMode(blu_ar)
