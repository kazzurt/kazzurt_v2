...
class colorwave6:
    def colorwave6(y):
        ...
        xf = x_off+num+(3*np.sin(rtim/10+np.pi)+6)
        yf = y_off-num+(3*np.sin(rtim/10)+7)
        if cyc==0:
            for i in arx:
...
#                         #sparkle = 1
        ...
        p[0,:] = (.5*(np.sin(rtim/10)**3)+.5)                  *coo[0]*viz_mf.flatMatHardMode(red_ar)
        p[1,:] = (.5*(np.sin(rtim/10+np.pi/3+np.pi/6)**3)  +.5)*coo[1]*viz_mf.flatMatHardMode(gre_ar)
        p[2,:] = (.5*(np.sin(rtim/10+np.pi*2/3+np.pi/3)**3)+.5)*coo[2]*viz_mf.flatMatHardMode(blu_ar)
